import csv, yaml, pymysql, smtplib, time, logging, inspect, sys, contextlib, unittest, io
import datetime
...
class CsvHelper(object):
    ...
    # 写入 csv 文件
    def write_list_into_csv(self, f, list_data):
        """
        list写入csv方法一：pandas
        将list存为csv文件，list的每一项代表csv文件的一行
        :param f: 文件路径
        :param list_data: 列表数据
        :return:
        """
        # 如果list中带有列名，则用list的第一个数据作为列名columns
        dataframe = pd.DataFrame(columns=list_data[0], data=list_data[1:])
        # 如果list中未带有列名，则需对list的第一行数据命名，不然列名显示0,1,2,3...
        # columns = ['属性1名称', '属性2名称', '属性3名称', ...]

        # 将整合的数据写入csv文件
        dataframe.to_csv(f, index=False, sep=',')

    def list_join_csv(self, f, mode, list_data):
        """
        list写入csv方法二：csv
        :param f: 文件路径
        :param list_data: 列表数据
        ...
        :return:
        """
        with open(f, mode=mode, newline='') as f_csv:
            csv_data = csv.writer(f_csv)
            csv_data.writerows(list_data)

    def dict_join_csv(self, f, mode, headers, dict_data):
        """
        dict写入csv
        :param f: 文件路径
        :param mode: 标示符
        :param headers: 第一行的列名
        :param dict_data: 字典数据
        :return:
        """
        with open(f, mode=mode, newline='') as f_csv:
            csv_data = csv.DictWriter(f_csv, headers)
            csv_data.writeheader()
            csv_data.writerows(dict_data)

    def column_into_csv(self, f, pattern):
        """
        将多个属性列存入csv文件。每一个属性列代表csv文件的一列。
　　    属性列表1=[属性值a，属性值b，属性值c，……]，属性列表2=[属性值A，属性值B，属性值C，……]
        ...
        """
        # pattern 的填入模式
        # pattern = {"属性名称1":list1,"属性名称2":list2,……}
        # 数据整合
        dataframe = pd.DataFrame(pattern)
        dataframe.to_csv(f, index=False, sep=',')
...
class YamlHelper(object):
    """读取yaml文件为dict"""
    def read_yaml_as_dict(self, f):
        """
        获取所有配置
        :param f: 文件路径
        :return:
        """
        with open(f, mode='r', encoding='uft-8') as file:
            dict_data = yaml.load(file.read())
            return dict_data
...
class DbHelper(object):
    ...
    def read_sql(self, f, encoding='utf-8'):
        """
        从文件中读取 sql 脚本
        :param f: 文件路径
        :param encoding: 字符集
        :return:
        """
        with open(f, 'r', encoding=encoding) as sql_file:
            sql = sql_file.read()
            return sql
    ...
    def execute_sql(self, sql):
        ...
        result = {'count': row_count, 'data': rows_data}
        cursor.close()
        return result
...
class Email(object):
    ...
    def email_attachment(self, file):
        try:
            ...
            # 收、发件人
            sender = '873119216@qq.com'
            password = '邮箱的网络授权码'
            receiver = '873119216@qq.com'

            msg = MIMEMultipart()
            msg['from'] = sender
            msg['to'] = ';'.join(receiver)
            msg['subject'] = '这个是xxx项目自动化测试报告主题'
...
class BoxDriver(object):
    ...
    def __init__(self, browser_type='c', Appdevice_config=0, download_path=r'd:\tester\Downloads', by_char=',', profile=None):
        """
        构造方法，实例化 Boxdriver 时候使用
        :param browser_type: 浏览器类型
        :param Appdevice_config: 如果 browser_type 传 app，则Appdevice_config需要传参数
        :param download_path: 下载保存路径
        :param by_char: 分隔符，默认使用 ","
        :param profile:
                    可选择的参数，不传递默认为None
                    如果传递一个profile，会按照预先的设定启动火狐
                    去掉遮挡元素的提示框等
        """
        self._by_char = by_char
        if browser_type == 'c':         # google浏览器
            profile = webdriver.ChromeOptions()
            # profile.add_experimental_option('excludeSwitches', ['ignore-certificate-error'])
        ...
        elif browser_type == 'f':       # firefox浏览器
            # if profile is not None:
            # profile = FirefoxProfile(profile)
            profile = webdriver.FirefoxProfile()
            profile.set_preference('browser.download.dir', download_path)       # 指定下载路径
            profile.set_preference('browser.download.folderList', 2)        # 2--使用自定义下载路径，0--下载到桌面，1--下载到默认路径
            profile.set_preference('browser.download.manager.showWhenStarting', False)      # 在开始下载时是否显示下载管理器
            profile.set_preference('browser.helperApps.neverAsk.saveToDisk', 'application/zip')     # 对所给出文件类型不再弹出框进行询问
            driver = webdriver.Firefox(firefox_profile=profile)
        elif browser_type == 'i':
            driver = webdriver.Ie()
        elif browser_type == 'app':
            # 手机测试设备信息参数化
            DEVICES_SELECTOR = YamlHelper().read_yaml_as_dict(r'biz\devicesConfig.yaml')[Appdevice_config]
            desired_caps = {'platformName': DEVICES_SELECTOR['platformName'],
                            'deviceName': DEVICES_SELECTOR['deviceName'],
                            'platformVersion': DEVICES_SELECTOR['platformVersion'],
                            'appPackage': DEVICES_SELECTOR['appPackage'],       # apk包名
                            'appActivity': DEVICES_SELECTOR['appActivity'],     # apk的launcherActivity
                            'noRest': True,     # 不重新安装apk
                            'unicodeKeyboard': True,        # 使用unicode编码方式发送字符串
                            'resetKeyboard': True}      # 将键盘隐藏起来
            driver = app_webdriver.Remote('http://127.0.0.1:4723/wd/hub', desired_caps)
            time.sleep(8)       # 页面加载等待8秒
        else:
    ...
    def _locate_elements(self, selector):
        """
        通过 selector 去定位元素，适用于定位的元素具有多个相同、不唯一
        :param selector:
        :return:
        """
        locator = self._convert_selector_to_locator(selector)
    ...
    def remove_cookie(self, name):
        ...
        if old_cookie_value is not None:        # 检查 cookie 是否存在，存在就移除
            self._base_driver.delete_cookie(name)

    """浏览器本身相关方法"""
    def refresh(self, url=None):
        """
        刷新页面，url若为空值，则刷新当前页面；不为空，则刷新指定页面
        :param url: 链接
        :return:
        """
        if url is None:
    ...
    def go_back(self):
        """物理键返回"""
        self._base_driver.keyevent(4)

    def swipe(self, x1, y1, x2, y2, t, n):
        """
        2 点坐标滑动
        :param x1: 第一个坐标的x轴
        :param y1: 第一个坐标的y轴
        ...
        """
        for i in range(n):
            self._base_driver.swipe(x1, y1, x2, y2, t)
    ...
    def swipe_up(self, t=500, n=1):
        ...
        :return:
        """
        s = self._base_driver.get_window_size()
        x1 = s['width'] * 0.5  # x 坐标
        y1 = s['height'] * 0.75  # 起始 y 坐标
        y2 = s['height'] * 0.25  # 终点 y 坐标
        for i in range(n):
    ...
    def swipe_down(self, t=500, n=1):
        ...
        s = self._base_driver.get_window_size()
        x1 = s['width'] * 0.5  # x 坐标
        y1 = s['height'] * 0.25  # 起始 y 坐标
        y2 = s['height'] * 0.75  # 终点 y 坐标
        for i in range(n):
            self._base_driver.swipe(x1, y1, x1, y2, t)
    ...
    def swipe_left(self, t=500, n=1):
        ...
        """
        s = self._base_driver.get_window_size()
        x1 = s['width'] * 0.75  # 起始 x 坐标
        y1 = s['height'] * 0.5  # y 坐标
        x2 = s['width'] * 0.25  # 终点 x 坐标
        for i in range(n):
    ...
    def swipe_right(self, t=500, n=1):
        ...
        """
        s = self._base_driver.get_window_size()
        x1 = s['width'] * 0.25  # 起始 x 坐标
        y1 = s['height'] * 0.5  # y 坐标
        x2 = s['width'] * 0.75  # 终点 x 坐标
        for i in range(n):
            self._base_driver.swipe(x1, y1, x2, y1, t)
    ...
    def input_text(self, selector, text):
        ...
        :return:
        """
        el = self._locate_element(selector).clear()
        el.send_keys(text)
    ...
    def clicks(self, selector, num):
        ...
        :return:
        """
        els = self._locate_elements(selector)[num]
        els.click()
    ...
    def click_by_enter(self, selector):
        ...
        self._locate_element(selector).send_keys(Keys.ENTER)
    ...
    def submit(self, selector):
        ...
        self._locate_element(selector).submit()
    ...
    def get_value(self, selector):
        """返回元素的 value"""
        return self._locate_element(selector).get_attribute('value')
    ...
    def get_attribute(self, selector, attribute):
        ...
        """
        return self._locate_element(selector).get_attribute(attribute)

    def get_text(self, selector):
        """Get element text information"""
        return self._locate_element(selector).text

    def get_displayed(self, selector):
        """Gets the element to display,The return result is true or false"""
        return self._locate_element(selector).is_displayed()

    def get_exist(self, selector):
        """判断元素是否存在，存在返回flag=true，否则返回false"""
        flag = True
        try:
    ...
    def get_selected(self, selector):
        ...
        """
        return self._locate_element(selector).is_selected()

    def get_text_list(self, selector):
        """根据 selector 获取多个元素，取得元素的 text 列表"""
    ...
    * 如果弹框元素无法查看，则使用如下方法可以搞定
    ...
    def save_window_snapshot(self, file_name):
        ...
        :return:
        """
        self._base_driver.save_screenshot(file_name)

    def save_screenshot(self, i=0):
        """
        截图，以方法的名字命名保存在文件夹
        :param i:保存多图时，图片后面的序号
        :return:
        """
        contents = inspect.stack()[1][3]
        content = contents+('%d' % i)
        self._base_driver.get_screenshot_as_file(u"D:\\yanling\\A8\\image\\{}.png".format(content))
    ...
    def save_element_snapshot_by_io(self, selector):
        """控件截图"""
        return self._locate_element(selector).screenshot_as_base64

    """等待方法"""
    def forced_wait(self, seconds):
        """
        强制等待
        :param seconds: 时间，单位 s
        :return:
        """
        time.sleep(seconds)
    ...
    def implicitly_wait(self, seconds):
        """
        隐式等待，Implicitly wait. All elements on the page.
    ...
    def upload_not_input(self, file, browser_type='Chrome'):
        """
        上传文件(标签不是 input 类型，使用 win32gui,得先安装 pywin32 依赖包, pip install pywin32)
        :param file: 将要上传的文件（绝对路径）
        ...
        # print(upload.get_attribute('value'))      # 获取属性
    ...
    def assert_new_record_exist_mysql(self, db_yaml_path, db_yaml_name, sql_file_path, select_field_num,
        ...
        :return: True / False
        """
        ydata = YamlHelper().read_yaml_as_dict(db_yaml_path)
        ...
        result = db_helper.execute_sql(sql)['data']
        db_helper.close()
        try:
            for i in result:
...
class TestRunner(_TemplateReport):
    ...
    def get_report_attributes(self, result):
        ...
        if status:
            ...
            status = 'none'
        return [('Start Time 开始时间', startTime),
